...
function Reference({ searchResults }: any) {
    const [commonKeys, setCommonKeys] = useState<string[]>([]);
    let resultKeys: string[], referenceKeys: string[] = [];
    ...
    useEffect(() => {
        if(searchResults?.data?.[0]) {
            ...
            referenceKeys= reference.map((field) => {return field.name});
          
          let res = findCommonKeys(resultKeys, referenceKeys);
  ...
  return (
    <div id='reference'>
      <span className='item-header'><Pin />Reference</span>
                    ...
                    <article key={item}>
...

                        {
                            ...
                            <p className='description'>
                                {reference.filter(obj => obj.name === item)[0].description}
                            </p>
                        }
